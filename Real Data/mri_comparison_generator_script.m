...
imagesc(reshape(sub_infos_TeCPSGD.I(:,frame1),[rows,cols]),[a,b]);
colormap(gray);axis image;axis off;
title('Ground Truth');


subplot_tight(2, 4, 2,[0.01,.01]);
imagesc(reshape(sub_infos_TeCPSGD.L(:,frame1),[rows,cols]),[a,b]);
colormap(gray);axis image;axis off;
title('Online-SGD');


subplot_tight(2, 4, 3,[0.01,.01]);
imagesc(reshape(sub_infos_olstec.L(:,frame1),[rows,cols]),[a,b]);
colormap(gray);axis image;axis off;
title('OLSTEC');

subplot_tight(2, 4, 4,[0.01,.01]);
imagesc(reshape(sub_infos_brst.L(:,:,frame1),[rows,cols]),[a,b]);
colormap(gray);axis image;axis off;
title('Proposed');

subplot_tight(2, 4, 5,[0.01,.01]);
imagesc(reshape(sub_infos_TeCPSGD.I(:,frame2),[rows,cols]),[a,b]);
colormap(gray);axis image;axis off;


subplot_tight(2, 4, 6,[0.01,.01]);
imagesc(reshape(sub_infos_TeCPSGD.L(:,frame2),[rows,cols]),[a,b]);
colormap(gray);axis image;axis off;

subplot_tight(2, 4, 7,[0.01,.01]);
imagesc(reshape(sub_infos_olstec.L(:,frame2),[rows,cols]),[a,b]);
colormap(gray);axis image;axis off;

subplot_tight(2, 4, 8,[0.01,.01]);
imagesc(reshape(sub_infos_brst.L(:,:,frame2),[rows,cols]),[a,b]);
colormap(gray);axis image;axis off;
