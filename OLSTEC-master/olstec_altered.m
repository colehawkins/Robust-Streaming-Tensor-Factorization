function [Xsol, infos, sub_infos] = olstec_altered(A_in, Omega_in, Gamma_in, tensor_dims, rank, X_init,base_stream, options)
% OLSTEC algorithm.
%
% Inputs:
...
% Modified by H.Kasai on Sep. 12, 2017
    ...
    sub_infos.err_residual_truth = 0;
    ...
    for outiter = 1 : maxepochs
        ...
        for k=1:slice_length
            ...
            if store_subinfo
                ...
                temp_for_norm = base_stream(:,:,k);
                norm_residual   = norm(temp_for_norm(:) - L_rec(:));
                error           = norm_residual/norm_I; 
                sub_infos.err_residual_truth = [sub_infos.err_residual_truth error];  
